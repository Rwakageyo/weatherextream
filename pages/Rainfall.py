...
st.title("Daily  Highest Rainfall Records ")
st.markdown("The table below indicates the daily breaking records the highest rainfall recorded at different weather stations across the country. The records are based on the entire available data for each station from when it started operation. ")
...
##st.header("Climate Visualization")
st.header("Rainfall extremes Records Chart")
st.bar_chart(data,x="Station_name", y="Value_mm")
##st.line_chart(data1,x="Station_name", y="Value_mm" ,use_container_width=True)

##st.area_chart(data1,x="Station_name", y="Value_mm")
